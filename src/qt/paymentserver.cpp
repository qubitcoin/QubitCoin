...
const QString BITCOIN_IPC_PREFIX("QubitCoin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click Qubit: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Qubit: click-to-pay handler");
    else
...
{
    // clicking on Qubit: URLs creates FileOpen events on the Mac:
